...
For a better looking scree plot function, I recommend `fviz_eig()` from [`factoextra` package](http://www.sthda.com/english/rpkgs/factoextra/){target="_blank"}. `fviz_eig()` has better looking outputs and more customizable plotting parameters, and since it is based on `ggplot2`, you can actually enhance it with the `ggplot2` syntax: `+`.
```{r}
library(factoextra)
